...
外壳：自绘无边框窗口（拖拽 / 最小化 / 最大化 / 关闭）、亮/暗主题、可拖拽调宽侧边栏（`Ctrl+B` 收成 56px 图标栏）、`Ctrl+K` 命令面板、工具收藏、全工具草稿持久化、CJK 字体自动加载。

工具：

| 工具 | 说明 |
|---|---|
| JSON 工具 | 语法着色（键 / 字符串 / 数字 / 字面量 / 标点分色），格式化 / 压缩 / 校验 / 转义 / 去转义（多层转义一次剥完）/ 键名排序，`Ctrl+F` 查找（边打边搜，Enter 跳下一处），行号槽折叠区块，缩进 2·4·Tab，撤销重做，长行自动换行（可关，关后横向滚动），**粘贴记录**（粘进来就自动存一条，右侧抽屉里按名称搜 / 点开看全文 / 复制 / 改名 / 恢复） |
| 文本 / 文件对比 | 逐行 diff，差异直接画在左右原文上（删除标左、新增标右，改动的字加深底色），**改完即时重比**（没有「对比」按钮），左右按 hunk 同步滚动，可跳上一处 / 下一处差异，可导出统一 diff 文本 |
...
### JSON 粘贴记录：为什么是文件不是数据库

往 JSON 工具的输入区粘东西，就**自动**存一条记录 —— 没有「保存」按钮，
因为这个功能的用处正是「刚才那份被覆盖了想翻回去」。

一条记录 = 一个文件：`<数据目录>/json-history/<名字>.json`。名字没起就用
`json-20260906-222015` 这种纯字母数字的自动名（它同时就是文件名，必须在任何
文件系统上都合法）；在抽屉里改名 = 改文件名，非法字符换成 `-`，Windows 的保留
设备名（`CON` / `NUL` / `COM1`…）加下划线避开。最多留 200 条，超出删最旧的。

选文件的理由：记录本身就是**文档**，名字就是文件名 —— 用户在「设置 → 数据 →
打开数据文件夹」里直接看到、拷得走、也能用别的工具改；增 / 删 / 改名 / 读全是
一次文件系统调用，用不到事务、索引与联表。引 `rusqlite` 要多编一份 C 的 SQLite，
六个平台的构建时间与安装包体积都得跟着涨，而它的能力这里一条都用不上。
元数据（时间、字节数）全部从文件系统现取，所以没有第二份需要同步的索引 ——
用户手动删掉一个文件，抽屉下次打开就是对的。

查看走**右侧抽屉**（工具条最右那个图标）：按名称搜索、改名、删除、清空。

**点一张卡片就在右半边看到那条记录的全文**（只读编辑区，着色 / 折叠 / 拖选
`Ctrl+C` 与正文一样，几 MB 的记录照样虚拟化），输入区**一个字都不动**；
确认是它再「恢复到输入区」，或者直接「复制」拿走。卡片上那行 110 字的缩略
只够认个大概 —— 原来唯一能看全的入口是「恢复到输入区」，而它会把手上
正在改的那份覆盖掉，于是「想看一眼」这件事根本没法做。
...
编辑区的复制沿用各家编辑器的语义：**有选区复制选区，没选区复制光标那一行**
（带行尾换行），`Ctrl+X` 同理但把整行连换行一起剪掉。要整篇请用工具条上的
「复制全文」—— 之前没选区时 `Ctrl+C` 复制的是整个文档，习惯性按一下就会把
几百 KB 全文塞进剪贴板，而人以为拿到的是一行。`Shift+Insert` / `Ctrl+Insert`
与 `Ctrl+V` / `Ctrl+C` 同义。

选区能超过一屏：**Shift+点**从原锚点扩到点的位置，**按住往编辑区外拖**会持续
自动滚动（离边越远滚得越快）。少了这两样，鼠标一次最多只能选到可视区的
最后一行 —— 想复制一段几百行的内容只能 `Ctrl+A` 全选。

粘贴进来的 CRLF 一律归成 LF。Windows 的剪贴板永远给 CRLF，原样插进去等于给
每行末尾塞一个看不见的 `\r`：它会跟着粘贴记录落盘、跟着「复制全文」再传出去。

视口最多滚到**最后一屏**，不是最后一行。整份粘贴（`Ctrl+A`、`Ctrl+V`）之后
光标停在末行，再把末行顶到第一排的话，屏幕上就只剩一行正文加一大片空白 ——
看起来就是「没粘全」。

查找条（`Ctrl+F`）是浮在编辑区右上角的一颗 400px 药丸，输入 / 计数 /
上一处 / 下一处 / 关闭都在同一个圆角容器里：开合不挤动布局，也不会让一个
搜三个字符的输入框铺满整个宽度。

### 给正文腾地方

1280×800 的窗口里，JSON 工具原来只有 30 行正文：标题条 60px + 说明条 44px +
编辑区自己的「JSON · N 行」标题行 24px + 16px 内边距，全是常驻装饰。现在：

- 编辑器类工具（`wide`：JSON / 对比 / SQL / YAML）标题条压到 44px，
  那一句说明收进标题旁的 ⓘ（悬停可看）——「页面自我介绍」读过一次就不必
  再占一整条；
- 编辑区不再画自己的标题行，行数常驻工具条右端（校验状态里那个「· N 行」
  同时去掉，免得同一个数写两遍）；
- **`Ctrl+B` 把侧栏收成 56px 图标栏**（左下角那颗箭头同义）。收成 0 是错的：
  那样除了 `Ctrl+K` 就没有切工具的入口了 —— 图标栏里工具照样一点就切，
  悬停出名字，搜索那颗放大镜直接开命令面板。状态随设置一起持久化。

逐像素量：正文 30 行 → 35 行；侧栏收起后编辑区宽 986px → 1196px。
