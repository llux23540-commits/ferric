; ─────────────────────────────────────────────────────────────────────────────
; Ferric 定制版安装模板。
; 基线：cargo-packager v0.11.8 crates/packager/src/package/nsis/installer.nsi
;（CI 已把 cargo-packager 钉在同版本，模板占位符不会漂移）。
;
; 与上游唯一的差异：**升级 = 直接覆盖安装**。上游在检测到旧版本时会插一页
; 「已安装旧版，是否先卸载」—— 单二进制应用覆盖即完成升级，这一页除了打断
; 用户没有任何作用。补丁把「升级」分支改为跳过该页（与上游 passive 模式的
; SkipIfPassive 同一语义）；同版本重装与降级仍保留原有询问页作为防护。
; ─────────────────────────────────────────────────────────────────────────────
...
Function PageReinstall
  ...
  ; Upgrading
  ${ElseIf} $R0 == 1
    ; ── Ferric 补丁：升级不再询问「是否先卸载旧版」，跳过本页直接覆盖安装。
    ; Abort 在自定义页函数里 = 跳过该页（上游 SkipIfPassive 同款用法），
    ; PageLeaveReinstall 因此不会执行，旧版卸载器也就不会被拉起。
    Abort
  ; Downgrading
  ${ElseIf} $R0 == -1
