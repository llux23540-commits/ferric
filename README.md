...
| 工具 | 说明 |
|---|---|
| JSON 工具 | 语法着色（键 / 字符串 / 数字 / 字面量 / 标点分色），格式化 / 压缩 / 校验 / 转义 / 去转义（多层转义一次剥完）/ 键名排序，`Ctrl+F` 查找（边打边搜，Enter 跳下一处），行号槽折叠区块，缩进 2·4·Tab，撤销重做，长行自动换行（可关，关后横向滚动），**粘贴记录**（粘进来就自动存一条，右侧抽屉里按名称搜 / 改名 / 恢复） |
...
查看走**右侧抽屉**（工具条最右那个图标）：按名称搜索、恢复到输入区、改名、删除、清空。
