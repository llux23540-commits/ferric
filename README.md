...
| 工具 | 说明 |
|---|---|
| JSON 工具 | 语法着色（键 / 字符串 / 数字 / 字面量 / 标点分色），格式化 / 压缩 / 校验 / 转义 / 去转义（多层转义一次剥完）/ 键名排序，`Ctrl+F` 查找（边打边搜，Enter 跳下一处），行号槽折叠区块，缩进 2·4·Tab，撤销重做，长行自动换行（可关，关后横向滚动） |
| 文本 / 文件对比 | 逐行 diff，差异直接画在左右原文上（删除标左、新增标右，改动的字加深底色），左右按 hunk 同步滚动，可跳上一处 / 下一处差异，可左右互换，可导出统一 diff 文本 |
| 时间戳 | Unix ↔ 日期时间，秒/毫秒，目标时区是可搜索的下拉（全量 597 个，支持中文名），自动识别多种日期格式 |
| JSON → YAML | JSON 转 YAML，实时校验 |
| SQL 格式化 | 格式化 / 压缩为单行；关键字**大写 ⇄ 小写**两档随时切，内容还是上次排版的原样就当场重排 |
| UUID 生成器 | UUID v4 / v7 / v6 / v5（命名空间），大小写 / 无连字符，Raw / JSON，执行历史；输出是只读编辑区 —— 鼠标拖选任意一段、双击选整行、`Ctrl+C` 复制 |
...
**检查失败不进侧栏**：侧栏左下角只在「下载中」与「可安装」两档出现标识。
失败是后台自动检查的结果，用户没要求过这件事 —— 断网时每次启动都挂一颗
红胶囊在版本号旁边，只会让人焦虑又无从处理。失败详情仍在两处可查：
设置 → 更新的状态行、以及操作记录。
...
进页面即自动拉取列表（启动就停在市场页时也拉 —— 否则那一页永远空着）。

列表用标签过滤：`全部 N` / `已安装 M`，计数跟着搜索词走（标签上的数字始终等于点进去
看到的条数）。搜索**打字即时生效**（本地过滤已拉到的列表），回车才向服务端重查；
正在装的那张卡自己描边并在卡底走进度，排队中的标「排队中」——
列表一长，只靠页顶那一条进度条就看不出到底在装哪一个。
...
代码折叠也在这一层：行号槽左边那列箭头按「行末的 `{` / `[`」判断可折叠
（判定只看行末，`"a": {},` 这种一行写完的不算），点开时才去扫配对的括号。
折叠**只改视图**，rope 一个字符都不动 —— 格式化、搜索、复制看到的仍是整篇。
折叠之后「第 N 个视觉行」不再等于「文档第 N 行」，行号、光标、选区、
滚动条刻度全部走 `TextBuffer::visible_rows` 这一处换算。

横向坐标不是「一个字符一格」：等宽字体只对拉丁字形等宽，中文由回退字体渲染、
步进宽一截。所以光标 / 选区 / 字符级高亮的 x 全部按**窄字宽的倍数**（浮点）
从 Rust 侧算好，宽字符按 UI 侧量出来的实际比值（同字号量 `0` 与 `字`）计。
按字符数算的后果是中文行上「点在这里、光标画在那里、字插到第三个地方」。
